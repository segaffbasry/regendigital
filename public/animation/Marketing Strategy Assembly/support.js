...
(() => {
  ...
  function init() {
    ...
    const postAnimationStatus = (type) => {
      if (!embedded || readyPosted || window.parent === window) return;
      readyPosted = true;
    ...
    const checkAnimationReady = () => {
      if (!embedded || readyPosted) return;
      const root = document.getElementById("dc-root");
      if (!root) return;
      if (root.querySelector(".sc-logic-error, .sc-placeholder-error")) {
        postAnimationStatus("__dc_animation_failed");
        return;
      }
      const visual = root.querySelector(
        "[data-om-exportable-video-with-duration-secs], [data-screen-label], [data-om-starter='animations-v3']"
      );
      if (!visual || root.querySelector(".sc-placeholder") || readyPending) return;
      ...
      Promise.all(images.map((image) => image.complete ? Promise.resolve() : new Promise((resolve) => {
        ...
        if (root.querySelector(".sc-logic-error, .sc-placeholder-error")) {
          postAnimationStatus("__dc_animation_failed");
        } else if (!root.querySelector(".sc-placeholder")) {
          postAnimationStatus("__dc_animation_ready");
        }
      })));
    };
    if (embedded) {
      readyObserver = new MutationObserver(checkAnimationReady);
      readyObserver.observe(document.documentElement, { childList: true, subtree: true });
      window.addEventListener("load", checkAnimationReady, { once: true });
    }
    const notifyHost = () => {
      if (window.parent === window) return;
    ...
    if (embedded) queueMicrotask(checkAnimationReady);
  }
  hideRawTemplate();
